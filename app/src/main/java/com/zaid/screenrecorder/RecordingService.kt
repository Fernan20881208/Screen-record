...
class RecordingService : Service() {
    ...
    private fun startRecording(intent: Intent) {
        val config = RecordingConfig(fps = intent.getIntExtra(EXTRA_FPS, 60), audioMode = AudioMode.INTERNAL)
        ...
        worker.execute {
            try {
                ...
                val audio = runCatching { audioEngine.start(resolved.config, tempAudio) }.getOrElse { com.zaid.screenrecorder.audio.AudioSelection(null, null, "Audio start failed: ${it.message}") }
    ...
    private fun stopRecording() {
        ...
        worker.execute {
            try {
                ...
                AppState.update(RecordingStatus(false, stats.durationMs, current.config, stats.averageFps, stats.droppedFrames, "Saved ${finalFile.name}; avg ${"%.2f".format(stats.averageFps)} FPS; ${stats.droppedFramesSource}"))
